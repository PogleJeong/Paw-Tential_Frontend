...
const ModifyFeedModal = ({show, onHide, seq}) => {
    ...
    const loadPost = async () => {
        axios.get("http://localhost:3000/getFeedInfo", {params:{"seq":seq}})
        .then(function(res){
            setContent(res.data.loadPost.content);
        })
        .catch(function(err){
            alert(err);
        })
    ...
    const modifyFeed = (e) => {
        ...
        axios.post("http://localhost:3000/feedUpdate", formData)
        .then(function(res){
            alert(res.data);
