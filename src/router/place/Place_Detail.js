...
const Place_detail = () => {
    ...
    function getReviewlist(address) {
        axios
            .get("http://localhost:3000/placeReview", { params: { location: address } })
            .then(function(resp) {
    ...
    const getCommentList = async (seq) => {
        try {
            const response = await axios.get(
                "http://localhost:3000/home/getCommentList",
                { params: { feedSeq: seq } }
            );
    ...
    const handleClick = async (seq) => {
        try {
            const response = await axios.get("http://localhost:3000/home/loadPost", {
                params: { seq: seq },
            });
    ...
    useEffect(() => {
        ...
        geocoder.addressSearch(location.state.address, function(result, status) {
            ...
            if (status === kakao.maps.services.Status.OK) {
                ...
                if (location.state.categorycode === "FD6") {
                    // 식당
                    imageSrc = "https://i.imgur.com/8BSGiMu.png";
                } else if (location.state.categorycode === "CE7") {
                    // 카페
                    imageSrc = "https://i.imgur.com/lmt1poz.png";
                } else if (location.state.category.includes("반려견놀이터") === true) {
                    // 놀이터
                    imageSrc = "https://i.imgur.com/bSXM4Te.png";
                } else if (location.state.category.includes("반려동물용품") === true) {
                    // 쇼핑
                    imageSrc = "https://i.imgur.com/AKEWxnR.png";
                } else if (location.state.categorycode === "AD5") {
                    // 숙박
                    imageSrc = "https://i.imgur.com/SCvEyM0.png";
                } else if (location.state.categorycode === "HP8") {
                    // 병원
        ...
        <>
            {feedDetailModal && (
                <FeedDetailModal
                    ...
                    onHide={() => {
                        setFeedDetailModal(false);
                    }}
                    feedData={feed}
                    photo={photo}
                    noPhoto={noPhoto}
                    getComment={() => {
                        getCommentList(feed.seq);
                    }}
                />
            )}
